...
import multicampusAPI, { StaffTransfer, ApproveTransferDTO } from '@/api/multicampus.api';
import PermissionGuard from '@/components/common/PermissionGuard';

const { Option } = Select;

const StaffTransferList: React.FC = () => {
  ...
  const fetchTransfers = async () => {
    ...
    try {
      ...
      message.error(error.message || 'Failed to fetch staff transfers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = (id: string) => {
    Modal.confirm({
      title: 'Approve Staff Transfer',
      content: 'Are you sure you want to approve this staff transfer?',
      okText: 'Yes, Approve',
      okType: 'primary',
      cancelText: 'Cancel',
      onOk: async () => {
        try {
          ...
          await multicampusAPI.approveStaffTransfer(id, approveData);
          message.success('Staff transfer approved successfully');
          fetchTransfers();
        } catch (error: any) {
          message.error(error.message || 'Failed to approve staff transfer');
        }
      },
    });
  };
  ...
  const handleReject = (id: string) => {
    Modal.confirm({
      title: 'Reject Staff Transfer',
      content: 'Are you sure you want to reject this staff transfer?',
      okText: 'Yes, Reject',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        try {
          ...
          await multicampusAPI.approveStaffTransfer(id, rejectData);
          message.success('Staff transfer rejected');
          fetchTransfers();
        } catch (error: any) {
          message.error(error.message || 'Failed to reject staff transfer');
        }
      },
    });
  };
  ...
  const getTransferTypeTag = (type: string) => {
    const typeConfig: Record<string, { color: string; text: string }> = {
      ...
      deputation: { color: 'purple', text: 'Deputation' },
    };
  ...
  const columns = [
    ...
    },
    {
      ...
      filters: [
        ...
        { text: 'Deputation', value: 'deputation' },
      ],
    },
    {
    ...
    },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: any, record: StaffTransfer) => (
        <Space size="middle">
          <Button
            type="link"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/multicampus/staff-transfers/${record.id}`)}
          >
            View
          </Button>
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        <Space style={{ width: '100%', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          ...
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => navigate('/multicampus/staff-transfers/new')}
          >
            New Transfer
          </Button>
        </Space>
...
export default StaffTransferList;
